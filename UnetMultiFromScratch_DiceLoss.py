...
def train(args, model, train_loader, test_loader, device, restart=False):
    ...
    criterion = lambda input, target: dice_loss(
        torch.softmax(input, dim=1).float(),
        torch.nn.functional.one_hot(target, args.num_classes).permute(0, 3, 1, 2).float(),
        multiclass=True
    )   
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        logging.info(f"Learning Rate at Epoch {epoch+1}/{args.epochs}: {current_lr:.6f}")
