"""
    ...
    Problem:1)
    Model Works Fine if restart=false but with restart=true needed to drop keys
...
def train(args, model, train_loader, test_loader, device, restart=False):
    ...
    if restart:
        ...
        # # Handle DataParallel mismatches
        # if list(state_dict.keys())[0].startswith("module.") and not isinstance(model, torch.nn.DataParallel):
        #     print("Converting DataParallel checkpoint to single-GPU model.")
        #     new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        #     state_dict = new_state_dict
        # elif not list(state_dict.keys())[0].startswith("module.") and isinstance(model, torch.nn.DataParallel):
        #     print("Converting single-GPU checkpoint to DataParallel model.")
        #     new_state_dict = {f"module.{k}": v for k, v in state_dict.items()}
        #     state_dict = new_state_dict

        # Remove mismatched "outc" layer from the checkpoint (since we redefine it)
        # state_dict = {k: v for k, v in state_dict.items() if "outc" not in k}
        ...
        print(f"Restarting training from epoch {checkpoint['epoch'] + 1}")

        # Load the best IoU from the best model checkpoint
        if os.path.exists(args.best_model_path):
            # best_checkpoint = torch.load(args.best_model_path, map_location=device)
            best_checkpoint = torch.load(args.best_model_path, map_location=torch.device('cpu'),weights_only=False ) 
            best_iou = best_checkpoint.get('best_iou')
        else:
            best_iou = 0.0
    else:
        
        # print("Using pretrained weights from Carvana")

        # # ReWrapping to make sure that code runs on both gpu
...
def main(args):
    ...
    model = torch.hub.load('milesial/Pytorch-UNet', 'unet_carvana')
        ...
        # #Modify the output layer for your number of classes
    model.outc = nn.Conv2d(64, args.num_classes, kernel_size=1)
...
#/home/temp/DataDummy/
    ...
    parser.add_argument("--batch_size", type=int, default=52, help="Batch size for training (default: 32)")
